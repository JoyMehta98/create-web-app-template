...
const createProject = async (projectName) => {
  ...
  try {
    ...
    // console.log("[4/4] initializing git...");
...
const chooseTemplates = async () => {
  ...
  const { chooseTemplates } = await prompt([
    {
      type: "list",
      name: "chooseTemplates",
      message: "Please select the template you want",
      choices: [...directories, new Separator()],
    },
  ]);
...
const dependenciesUpdates = async () => {
  ...
  if (isUpdated) {
    const { isUpdatedReconfirm } = await prompt([
      {
        type: "confirm",
